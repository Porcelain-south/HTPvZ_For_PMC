    ...
    protected void apply(Map<ResourceLocation, JsonElement> map, IResourceManager manager, IProfiler profiler) {
        ...
        map.forEach((res, jsonElement) -> {
            try {
                ...
                final int chance = JSONUtils.getAsInt(jsonObject, "triggerChance", 10);//没有写就默认10
                ...
                array.forEach(e -> {
                    if(e.isJsonObject()) {
                        ...
                        final String placeType = JSONUtils.getAsString(obj, "placement", SpawnType.PlaceType.SNOW_AND_LAND.toString().toLowerCase());
