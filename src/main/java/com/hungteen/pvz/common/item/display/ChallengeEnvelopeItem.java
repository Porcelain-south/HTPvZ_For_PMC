...
import com.hungteen.pvz.client.render.challenge.ChallengeRangeRender;
...
public class ChallengeEnvelopeItem extends Item {
    ...
    private int getRadius(IChallengeComponent challengeComponent) {
        return challengeComponent.getOuterPlacement().getApproximateRadius() + 1;
    }
    ...
    public ActionResultType useOn(ItemUseContext context) {
        if (context.getItemInHand().getItem() instanceof ChallengeEnvelopeItem && context.getClickedFace() == Direction.UP) {
            ...
            if (context.getHand() == Hand.OFF_HAND) {
                if (context.getLevel().isClientSide) {
                    int range = getRadius(challengeComponent);
                    ChallengeRangeRender.showRangeCenter = context.getClickedPos();
                    ChallengeRangeRender.remainTicks = 20 * 30;
                    ChallengeRangeRender.showRangeRadius = range;
                }
