...
//        this.setPersistenceRequired();
    ...
    public static void randomInitSkills(AbstractPAZEntity pazEntity, int maxLevel){
        ...
        for(int i = 0; i < 10; ++ i){
            ...
            int lvl = 0;
            if(skillNBT.contains(skillType.getIdentity()))
            {
                lvl = skillNBT.getInt(skillType.getIdentity());
            }
            while(lvl < skillType.getMaxLevel() && skillType.getCostAt(lvl) <= point && pazEntity.getRandom().nextInt(skillType.getMaxLevel()*2) > lvl){
