...
public class TorchWoodEntity extends PVZPlantEntity {
	...
	protected void normalPlantTick() {
		...
		if(! level.isClientSide) {
			if(this.getFlameType() == FlameTypes.BLUE) {
				super_time--;
				if (super_time==0) setFlameType(FlameTypes.YELLOW);
			}
			this.heatPeas();
		}else {
	...
	public void readAdditionalSaveData(CompoundNBT compound) {
		...
		if(compound.contains("super_time")) {
			this.super_time = compound.getInt("super_time");
		}
	...
	public void addAdditionalSaveData(CompoundNBT compound) {
		...
		compound.putInt("super_time", this.super_time);
