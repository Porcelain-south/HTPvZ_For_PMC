...
public abstract class AbstractTombStoneEntity extends PVZZombieEntity {
	...
	protected int lifeRange = 120000;
	...
	public void tick() {
		if(! this.level.isClientSide && this.getVehicle() == null && this.onGround && (this.getDeltaMovement().x != 0 || this.getDeltaMovement().z != 0)) {
		...
		if(! this.level.isClientSide) {
			...
			else if(lifeRange < 20000 && (level.getDayTime() % 24000 < 12000 ? random.nextInt(100) == 0 : false) && level.getNearestPlayer(this, 20) == null)
			{
				this.remove();
			}
		}
