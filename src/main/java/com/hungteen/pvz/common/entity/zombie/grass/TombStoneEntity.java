...
public class TombStoneEntity extends AbstractTombStoneEntity {
	...
	protected int waveSummonTick = 28800;
		...
		if(! level.isClientSide) {//update wave spawn.
			if(this.waveSummonTick > 0) {
				...
				if(this.waveSummonTick == 1) {
					if(level.getDayTime() % 24000 < 15000)
					{
						this.waveSummonTick = 15000 - (int)(level.getDayTime() % 24000) + MathUtil.getRandomMinMax(getRandom(), 400, 2400);
					}
					else if (level.getDayTime() % 24000 > 22000)
					{
						this.waveSummonTick = 15000 + 24000 - (int)(level.getDayTime() % 24000) + MathUtil.getRandomMinMax(getRandom(), 400, 2400);
					}
					else
					{
						this.summonZombie();
						this.waveSummonTick = 14400;
					}
				}
			}
		}
