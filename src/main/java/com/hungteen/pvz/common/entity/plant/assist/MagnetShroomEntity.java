...
public class MagnetShroomEntity extends PVZPlantEntity {
	...
	public void startSuperMode(boolean first) {
		...
		for(LivingEntity target : level.getEntitiesOfClass(LivingEntity.class, EntityUtil.getEntityAABB(this, range, range), (entity) -> {
			...
			if(! (target instanceof IHasMetal) || (target instanceof IHasMetal && target instanceof JackInBoxZombieEntity && ((JackInBoxZombieEntity) target).isInExplosion())) continue;
	...
	public void dragMetal(LivingEntity target) {
		if(target instanceof IHasMetal && !(target instanceof JackInBoxZombieEntity && ((JackInBoxZombieEntity) target).isInExplosion())) {
			((IHasMetal) target).decreaseMetal();
