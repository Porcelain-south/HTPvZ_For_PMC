...
//		return new ZombiePathNavigator(this, world);
	...
	protected void initAttributes() {
		...
		this.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue(this.getEatDamage()+this.getSkillValue(SkillTypes.HIGH_EAT_DAMAGE));
		this.getAttribute(Attributes.FOLLOW_RANGE).setBaseValue(this.getFollowRange());
		this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(this.getWalkSpeed()*this.getSkillValue(SkillTypes.ZOMBIE_FAST_MOVE));
	...
	public void zombieTick() {
		...
		if(! this.level.isClientSide){
			...
			if (EntityUtil.isEntityStench(this) && this.getExistTick() % 20 == 0) {
	...
	protected boolean canRemoveWhenDeath() {
		return (ConfigUtil.enableZombieDropParts() && !level.isClientSide) || super.canRemoveWhenDeath();
	}
