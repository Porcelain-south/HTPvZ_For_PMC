...
public abstract class AbstractDaveShopContainer extends PVZContainer {
    ...
    public void onSell(int pos) {
        this.getValidGoods(player).forEach(item -> {
            if (item.getPos()==pos) {
                this.buyGood(item);
                this.daveOpt.ifPresent(dave -> {
                    dave.sellGoodForTransactions(item);
                    if (player instanceof ServerPlayerEntity) {
                        PVZTradeTrigger.INSTANCE.trigger((ServerPlayerEntity) player, dave, item.getGoodPrice());
                    }
                });
            }
        });
    ...
    public List<AbstractDaveEntity.GoodType> getValidGoods(PlayerEntity player) {
        if (this.daveOpt.isPresent()) {
            List<AbstractDaveEntity.GoodType> temp1 = this.daveOpt.get().getGoodList();
            List<AbstractDaveEntity.GoodType> temp2 = temp1.stream().filter(goodType -> {
