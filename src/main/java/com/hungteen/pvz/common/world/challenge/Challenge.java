...
public class Challenge implements IChallenge {
	...
	protected void tickWave() {
		...
		if(this.getWorld().getDifficulty() == Difficulty.HARD){
			if(this.tick % 10 == 2){
				...
				this.getPlayers().forEach(p -> {
					this.currentMaxLevel =Math.max(this.currentMaxLevel, PlayerUtil.getResource(p, Resources.TREE_LVL)) ;
				});
			}
		} else {
			this.currentMaxLevel = 0;
		...
		while(it.hasNext()) {
			...
			if (!entity.isAlive()||entity instanceof BungeeZombieEntity){
				it.remove();
			}
		}
	}
		...
		for(int i = 0; i < count; ++ i) {
			...
			if(entity != null) {
				...
				if(entity instanceof AbstractPAZEntity){//init skills.
					AbstractPAZEntity.randomInitSkills((AbstractPAZEntity) entity, this.currentMaxLevel+Math.max(0, (this.currentMaxLevel - this.getRaidComponent().getRecommendLevel())));
				}
			}
		}
	}
