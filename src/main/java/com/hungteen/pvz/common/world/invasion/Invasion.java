...
public class Invasion {
    ...
    public void tick() {
        ...
        if (this.isRunning() && this.getSpawnInvasion() != null && this.world.getDifficulty() != Difficulty.PEACEFUL && !player.isSpectator()) {
            ...
            if (this.currentWave < this.getTotalWaveCount())
            {
                if (time == this.getWaveTime(this.currentWave)) {
    ...
    public boolean currentSituation(TriPredicate<Integer, Integer, Integer> predicate){
        ...
        final int plantcount = (int) EntityUtil.getFriendlyLivings(player, EntityUtil.getEntityAABB(player, range, 255))
                .stream().filter(entity -> {
                    return entity instanceof PVZPlantEntity;
                }).count() + 1;
        final int zombiecount = EntityUtil
                .getPredicateEntities(player, EntityUtil.getEntityAABB(player, range, 255), MobEntity.class, e -> {
    ...
    public void spawnInvaders() {
        if (currentSituation((p, z, m) -> {
            ...
            return z < m*(0.2 + 0.8 * (p > 50 ? 1 : (p / 50)*(p / 50)));
            ...
            for (int i = 0; i < this.getSpawnCount(); ++i) {
                ...
                final BlockPos pos = WorldUtil.findRandomSpawnPos(world, player.blockPosition(), 10, 50, PVZConfig.COMMON_CONFIG.InvasionSettings.MaxSpawnRange.get(),
...
//		    }
    ...
    private boolean spawnZombieTeam(int cnt) {
        BlockPos mid = WorldUtil.findRandomSpawnPos(world, player.blockPosition(), 20, 55, 60,
    ...
    private void spawnInvader(SpawnType spawnType, BlockPos pos){
        ...
        if(InvasionManager.enableSkills(this.world) && entity instanceof AbstractPAZEntity){
            AbstractPAZEntity.randomInitSkills((AbstractPAZEntity) entity, this.invasionLvl/2 +Math.max(0, this.invasionLvl - spawnType.getInvasionLevel()));
        }
...
//        world.addFreshEntity(entity);
    ...
    public void resetWaveTime() {
        final int cnt = getPlayerWaveCount(player.getRandom(), this.invasionLvl);//10
        //not happen at the first 2000 ticks and the last 2000 ticks of the day.
        final int eachTime = 20000 / cnt;
        for (int i = 0; i < cnt; ++i) {//10 times
            final int offset = 2000 + i * eachTime + player.getRandom().nextInt(eachTime*4/5);
            final int pos = i;
